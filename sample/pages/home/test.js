...
var g_serviceMethodProvider = new CHBase.Sample.ServiceMethodProvider(g_appSettings);
startApp();
...
function createHealthVaultApp() {
    var app = new CHBase.Foundation.HealthVaultApp(g_appSettings);
...
function testItemFilter() {
    var filter = new CHBase.Types.ItemFilter();

    filter.effectiveDateMax = new CHBase.Types.DateTimeValue("12/3/2010");
    filter.updatedDateMin = new CHBase.Types.DateTimeValue("5/6/1993");

    displayText(filter.serialize());
}

function testCodableValue() {
    var code = new CHBase.Types.CodedValue("Snomed", "12346");

    var codableValue = new CHBase.Types.CodableValue("Lipitor", code);

    codableValue.codes.addIfDoesNotExist(code);
    code = new CHBase.Types.CodedValue("Snomed", "abcd");
    codableValue.codes.addIfDoesNotExist(code);

    var xml = codableValue.serialize();
    displayText(xml);

    codableValue = CHBase.Types.CodableValue.deserialize(xml);
...
function testMedication() {
    ...
    med = CHBase.ItemTypes.Medication.deserialize(xml);
...
function testCondition() {
    ...
    cond = CHBase.ItemTypes.Condition.deserialize(xml);
...
function testItemQuery() {
    ...
    var query = new CHBase.Types.ItemQuery();
    query.name = "Foo";
    query.view.typeVersions.append("33");
    query.itemIDs = new Array
    (
        CHBase.ItemTypes.Medication.typeID,
        CHBase.ItemTypes.Weight.typeID,
        CHBase.ItemTypes.Height.typeID
    );
    query.keys = new Array
    (
        new CHBase.Types.ItemKey("1234", "1.1")
    );

    query.maxResults = new CHBase.Types.NonNegativeInt(1000);
    query.maxFullItems = new CHBase.Types.NonNegativeInt(100);
...
function testLRUCache() {
    ...
    try {
        var maxItems = 10;
        var cache = new CHBase.Store.ObjectCache(maxItems, false);
        var maxTestItems = 15;
...
function putItemsInLocalStore() {
    ...
    for (i = 0; i < maxItems; ++i) {
        weight = makeWeight();
        weight.key = CHBase.Types.ItemKey.newKey();
...
function testSynchronizedStoreFor(typeID, awaitAll) {
    ...
    var log = beginLog();
    var record = getCurrentRecord();
    var filter = CHBase.Types.ItemFilter.filterForType(typeID);
    //
    ...
    store.getViewAsync(viewName).then(
        function (view) {
            if (view == null) {
                ...
                view = store.createView(viewName, CHBase.Types.ItemQuery.queryForTypeID(typeID));
...
function typeIDForSyncViewTest() {
    return CHBase.ItemTypes.BloodPressure.typeID;
}
...
function testVocab() {
    ...
    var vocabIDs = new Array(
        CHBase.ItemTypes.Medication.vocabForDoseUnits(),
        CHBase.ItemTypes.Medication.vocabForStrengthUnits()
    );
    ...
    g_hvApp.vocabs.getAsync(vocabIDs).then(
        ...
        function (error) {
            if (error.number == CHBase.Foundation.ServerErrorNumber.vocabNotFound) {
...
function testVocabSearch() {
    var rxNorm = CHBase.ItemTypes.Medication.vocabForName();
...
function testGetThings() {
    ...
    var filter = new CHBase.Types.ItemFilter(new Array(
        CHBase.ItemTypes.Condition.typeID,
        CHBase.ItemTypes.File.typeID,
        CHBase.ItemTypes.Medication.typeID,
        CHBase.ItemTypes.Procedure.typeID,
        CHBase.ItemTypes.Weight.typeID,
        CHBase.ItemTypes.Height.typeID
    ));
    query.filters.append(filter);
    query.maxResults = new CHBase.Types.NonNegativeInt(100);
    //query.maxFullItems = new CHBase.Types.NonNegativeInt(1);
...
function testQuery(query, fullItem) {
    ...
    //query.filters[0].updatedDateMin = new CHBase.Types.DateTime("8/17/2012");
...
function testQueryWithPendingGet(query, fullItem) {
    ...
    query.maxFullItems = new CHBase.Types.NonNegativeInt(0);  // Don't inline any things...
    displayText(query.serialize());
...
function testRemoveThings() {
    ...
    var query = CHBase.ItemTypes.Weight.queryFor();
...
function testOpenFile() {
    ...
    var query = CHBase.ItemTypes.File.queryFor();
    record.getAsync(query).then(
        function (itemList) {
...
function testUploadFile() {
    ...
    (
        function (storageFile) {
            var file = new CHBase.ItemTypes.File();
...
function testGetPersonalImage() {
    ...
    record.getAsync(CHBase.ItemTypes.PersonalImage.queryFor()).then(
        function (items) {
            /// <param name="items" type="CHBase.ItemTypes.ItemDataTypedList" />
            if (items.size > 0) {
...
function refreshPersonalImage(record, personalImage) {
    ...
    /// <param name="personalImage" type="CHBase.ItemTypes.PersonalImage" />
            ...
            record.getAsync(CHBase.ItemTypes.PersonalImage.queryFor()).then(
                function(items) {
                    ...
                    if (items.size > 0) {
                        ...
                        personalImage = new CHBase.ItemTypes.PersonalImage();
                    }
...
function testMakeBasic() {
    var record = getCurrentRecord();
    record.getAsync(CHBase.ItemTypes.BasicV2.queryFor()).then(function(items) {
        var basic;
        if (items && items.length > 0) {
            basic = items[0];
        } else {
            basic = new CHBase.ItemTypes.BasicV2();
        }

        basic.gender = 'f';
        basic.birthYear = new CHBase.Types.Year(1979);
        basic.city = 'Hollywood';
        basic.country = new CHBase.Types.CodableValue('United States');
        basic.firstDayOfWeek = new CHBase.Types.DayOfWeek(1);
        basic.postalCode = '90210';
        basic.state = new CHBase.Types.CodableValue('California');
        basic.languages = [new CHBase.Types.Language(new CHBase.Types.CodableValue('Piglatin'), true)];
...
function testMakePersonal() {
    var record = getCurrentRecord();
    record.getAsync(CHBase.ItemTypes.Personal.queryFor()).then(function (items) {
        var personal;
        if (items && items.length > 0) {
            personal = items[0];
        } else {
            personal = new CHBase.ItemTypes.Personal();
        }

        personal.name = new CHBase.Types.Name("First", "Middle", "Last");
        var birthDate = new CHBase.Types.StructuredDateTime();
        birthDate.date = new CHBase.Types.Date("1982", "1", "1");
        personal.birthDate = birthDate;
        personal.bloodType = new CHBase.Types.CodableValue("B+");
        personal.ethnicity = new CHBase.Types.CodableValue("Indian");
        personal.nationalIdentifier = "123-45-6789";
        personal.maritalStatus = new CHBase.Types.CodableValue("Single");
        personal.employmentStatus = "Employed";
        personal.isDeceased = new CHBase.Types.BooleanValue(false);
        var approxDateOfDeath = new CHBase.Types.ApproxDateTime();
        approxDateOfDeath.description = "Never";
        personal.dateOfDeath = approxDateOfDeath;
        personal.religion = new CHBase.Types.CodableValue("Buddhism");
        personal.isVeteran = new CHBase.Types.BooleanValue(true);
        personal.educationLevel = new CHBase.Types.CodableValue("phd");
        personal.isDisabled = new CHBase.Types.BooleanValue(false);
...
function testPutThingsRaw() {
    var allergy = new CHBase.ItemTypes.Allergy("Honey");
...
function testParseRaw() {
    ...
    var recordItem = CHBase.Types.RecordItem.deserialize(xml);
    displayText(recordItem.type.id);
}

function testGetThingTypes() {
    var parameters = new CHBase.Types.ThingTypeGetParams();
    CHBase.ItemTypes.ItemTypeManager.getItemTypeDefinitions(g_hvApp, parameters).then(
        function (results) {
            var text = '';
...
function testQueryPermissions(thingTypeId) {
    var record = getCurrentRecord();
    var queryParams = new CHBase.Types.QueryPermissionsRequestParams(thingTypeId);
...
function testSelectInstance() {
    var location = new CHBase.Types.Location();
    location.country = "gb";
    g_hvApp.selectInstanceAsync(location).then(function (result) {
        displayContentSafe(CHBase.Sample.Utility.toXml(result));
    });
}