...
function store_testChangeTable() {
    ...
    var log = beginLog();
    var changeTable;
    var key = CHBase.Types.ItemKey.newKey();
    store_createTestFolder().then(function (folder) {
        ...
        return changeTable.trackChangeAsync(CHBase.ItemTypes.Medication.typeID, key, HVStore.RecordItemChangeType.put);
    }).then(function () {
        writeLine(log, "track weight");
        return changeTable.trackChangeAsync(CHBase.ItemTypes.Weight.typeID, CHBase.Types.ItemKey.newKey(), HVStore.RecordItemChangeType.put);
    }).then(function () {
        writeLine(log, "removeChange");
        return changeTable.trackChangeAsync(CHBase.ItemTypes.Medication.typeID, key, HVStore.RecordItemChangeType.remove);
    }).then(function () {
...
function store_getBPType() {
    return store_getSyncType(CHBase.ItemTypes.BloodPressure.typeID);
}
...
function store_testNewWeightBackground() {
    ...
    store.types.getAsync(CHBase.ItemTypes.Weight.typeID).then(function (st) {
        syncType = st;
...
function store_testUpdate(updateDate) {
    ...
    store_getBPType().then(function (st) {
        ...
        if (updateDate) {
            item.when = CHBase.Types.StructuredDateTime.now();
        }
...
function store_testSync() {
    var store = getCurrentRecordStore();
    var syncType;
    store.types.getAsync(CHBase.ItemTypes.BloodPressure.typeID).then(function (st) {
        syncType = st;
        return st.synchronizeAsync();
...
function store_forceServerFailures(enabled) {
    ...
    if (enabled) {
        ...
        var statusCode = CHBase.Foundation.ServerErrorNumber;
        var errorCodes = [statusCode.failed, statusCode.requestTimedOut];
        var testStore = CHBase.Store.TestRemoteStore.createServerErrorProducer(remoteStore);
        ...
        store.data.remoteStore = new CHBase.Store.RemoteItemStore(store.data.record);
...
function store_testMultiSync() {
    ...
    var store = getCurrentRecordStore();
    var typeIDs = [ CHBase.ItemTypes.BloodPressure.typeID, 
                    CHBase.ItemTypes.Weight.typeID, 
                    CHBase.ItemTypes.BloodGlucose.typeID];
...
function store_testMultiRefresh() {
    ...
    store.types.getMultipleAsync(typeIDs).then(function (sTypes) {
        var refresher = new CHBase.Store.SynchronizedViewItemRefresher(store);
...
function testWriteBack()
{
    ...
    //store_testSyncType(CHBase.ItemTypes.BloodPressure.typeID);
    //store_testNew();
    store_testUpdate();
}